...
def search(request):    
    ...
    ########################################################################################################################################
    # If a searched keyword is provided, filter the references containing that keyword and replace all_references with filtered references #
    ########################################################################################################################################
    if search_keyword:
    ...
    #######################################################################################################################################
    # If a searched author is provided, filter the references by that author lastname and replace all_references with filtered references #
    #######################################################################################################################################
    elif search_author:
        all_references_ids = []
    ...
    #################################################################################################################################################
    # If a searched publisher is provided, filter the references containing that publisher name and replace all_references with filtered references #
    #################################################################################################################################################
    ...
    #################################################################################
    # If no specific searches, then all_references contains all existing references #
    #################################################################################       
    else:
    ...
    for row in all_references:
        ...
        ################################################################################################
        # Retrieve author informations based on reference_id (from table Author_Reference then Author) #
        ################################################################################################
        ...
        #########################################################
        # Retrieve publisher informations based on publisher_id #
        #########################################################
        ...
        #########################################
        # Retrieve genres based on reference_id #
        #########################################
        ...
        #############################################
        # Store all informations for each reference #
        #############################################
        reference = {
            "reference_id": row[0],
            "title": row[1],
    ...
    #####################################################################################################
    # Render the main_page.html template with the filtered references (if any) and the searched keyword #
    #####################################################################################################
    dictionary = {
...
def reference_detail(request, reference_id): #it takes reference_id from URL
    ...
    #######################################################################################
    # Retrieve genres names based on reference_id (from table Genre_Reference then Genre) #
    #######################################################################################
    genre_ids = promethee_cursor.execute('''SELECT keyword_id FROM Genre_Reference WHERE reference_id = ?''', (reference_id,)).fetchall()
    genres = []
    ...
    #############################################################
    # Retrieve reference informations based on the reference_id #
    #############################################################
...
def publisher_detail(request, publisher_id):
    ...
    ############################################
    #Retrieve references based on publisher_id #
    ############################################
    ...
    for single_reference in all_references:
        ########################################################################
        # Retrieve references informations based on unique reference_id (loop) #
        ########################################################################
...
def author_detail(request, author_id):
    ...
    ##############################################
    # retrieve author details based on author_id #
    ##############################################
    ...
    #######################################################
    # Retrieve country informations based on country_code #
    #######################################################
    country_code = all_informations[3]
    ...
    ##########################################
    # Retrieve references based on author_id #
    ##########################################
    ...
    for single_reference_id in reference_ids:
        reference_id = single_reference_id[0]
        ################################################
        # Retrieve genres based on unique reference_id #
        ################################################
        genre_ids = promethee_cursor.execute('''SELECT keyword_id FROM Genre_Reference WHERE reference_id = ?''', (reference_id,)).fetchall()
        genres_list = []
        for single_genre_id in genre_ids:
        ...
        ####################################################################################
        # Retrieve author informations based on reference_id (from table Author_Reference) #
        ####################################################################################
        authors_id_position = promethee_cursor.execute('''SELECT author_id, position FROM Author_Reference WHERE reference_id = ?''', (reference_id,)).fetchall()
        ...
        ###########################################################
        # Retrieve reference details based on unique reference_id #
        ###########################################################
...
def genres(request):
    ...
    promethee_cursor = promethee_conn.cursor()
    ####################################################
    # Retrieve all genres and keyword ordered from A-Z #
    ####################################################
    ...
    for single_genre in all_genres:
        ...
        genres.append(genre)
    #########################################
    # If we searched for a specific choice  #
    #########################################
    searched_keyword_id = request.GET.get('choice') #return keyword_id
    if searched_keyword_id:
        choice_name = promethee_cursor.execute('''SELECT keyword
                                               FROM Genre
                                               WHERE keyword_id = ?''', (searched_keyword_id,)).fetchall()[0][0]
        ###################################################
        # Retrieve all references_ids based on keyword_id #
        ###################################################        
        references_ids = promethee_cursor.execute('''SELECT reference_id
                                                  FROM Genre_Reference
                                                  WHERE keyword_id = ?''', (searched_keyword_id,)).fetchall()
        ...
        for single_reference_id in references_ids:
            ...
            for single_author_id_position in authors_id_position:
                ...
                author_informations.append(single_author_informations)
            ###############################################################
            # Retrieve reference informations for a specific reference_id #
            ###############################################################
    ...
    if request.method == "POST":
        ...
        ###################################################
        # Check if the new genre exists in the DB already #
        ###################################################
...
def country_detail(request):
    ...
    ###########################################
    # Retrieve all countries ordered from A-Z #
    ###########################################
    ...
    if searched_country_code:
        ######################################
        # If we don't click on update button #
        ######################################
        if not show_update_form:
            ##############################################
            # Retrieve all authors based on country_code #
            ##############################################
        ...
        #####################################
        # If we have click on update button #
        #####################################
    ...
    if request.method == "POST":
        ...
        ###################################################
        # Check if the new country exists in the DB already #
        ###################################################
        if check_if_exists_already:
...
def add_reference(request):
    ...
    #############################################################
    # Retrieve all authors and author_lastname ordered FROM A-Z #
    #############################################################
    ...
    ###############################################################
    # Retrieve all publishers and publisher_name ordered from A-Z #
    ###############################################################
    ...
    ###################################################
    # Retrive all genres and keyword ordered from A-Z #
    ###################################################
    all_genres = promethee_cursor.execute('''SELECT keyword_id, keyword
                                          FROM Genre
                                          ORDER BY keyword ASC''').fetchall()
    ...
    ###################################################
    # If a quantity of authors is entered in the form #
    ###################################################
    ...
    ####################################################
    # If form is filled and quantity of authors in URL #
    ####################################################
...
def add_publisher(request):
    ...
    ###############################################################
    # Retrieve all publishers and ordered publisher_name from A-Z #
    ###############################################################
    all_publishers = promethee_cursor.execute('''SELECT publisher_id, publisher_name
                                              FROM Publisher
                                              ORDER BY publisher_name ASC''').fetchall()
    ...
    ####################################
    # If we enter a new publisher name #
    ####################################
    if request.method == 'POST':
        ...
        ##################################################
        # If it already exists, returns an error message #
        ##################################################
...
def add_author(request):
    ...
    #############################################################
    # Retrieve all authors and ordered author_lastname from A-Z #
    #############################################################
    all_authors = promethee_cursor.execute('''SELECT author_id, author_firstnames, author_initials,
                                           author_lastname, country_code, affiliation, author_email
                                           FROM Author
    ...
    ##############################################
    # If we have entered new author informations #
    ##############################################
    if request.method == 'POST':
        ...
        #####################################
        # We split firstnames into initials #
        #####################################
        ...
        #########################################################
        # We check if the author already exists in the database #
        #########################################################
        if check_if_author_already_exists:
            ...
            ###############################################
            # If country_code doesn't exist yet in the DB #
            ###############################################
...
def update_reference(request, reference_id):
    ...
    ####################################################
    # Retrieve all genres and ordered keyword from A-Z #
    ####################################################
    ...
    ############################################################################################
    # Retrieve all genres based on unique reference_id (from table Genre_Reference then Genre) #
    ############################################################################################
    ...
    ################################################################
    # Retrieve reference informations based on unique reference_id #
    ################################################################
    ...
    #############################################
    # If we have updated reference informations #
    #############################################
    if request.method == "POST":
...
def update_publisher(request, publisher_id):
    ...
    ############################################
    # If we have entered an new publisher name #
    ############################################
    if request.method == "POST":
        new_publisher_name = request.POST.get('new_publisher_name')
...
def update_author(request, author_id):
    ...
    ##########################################################
    # Retrieve author informations based on unique author_id #
    ##########################################################
    ...
    ##########################################
    # If we have updated author informations #
    ##########################################
    if request.method == "POST":
        ...
        ###################################
        # Handle firstnames into initials #
        ###################################
