...
def search(request):    
    ...
    if search_keyword:
        ...
        all_authors_informations = promethee_cursor.execute('''SELECT author_id, author_initials, author_lastname, country_code, affiliation, author_email FROM Author WHERE author_lastname LIKE ?''', (search_author,)).fetchall()
        all_references_ids = []
        for single_author_information in all_authors_informations:
            author_id = single_author_information[0]
            
            single_author = {
                'author_id': single_author_information[0],
                'author_initials': single_author_information[1],
                'author_lastname': single_author_information[2],
                'country_code': single_author_information[3],
                'affiliation': single_author_information[4],
                'author_email': single_author_information[5]
            }
            filtered_authors.append(single_author)
        ...
        all_publishers_informations = promethee_cursor.execute('''SELECT publisher_id, publisher_name FROM Publisher WHERE publisher_name LIKE ?''', ('%{}%'.format(search_publisher),)).fetchall()
        all_references_ids = []
        for single_publisher_information in all_publishers_informations:
            publisher_id = single_publisher_information[0]
            ...
            single_publisher = {
                'publisher_id': single_publisher_information[0],
                'publisher_name': single_publisher_information[1]
            }
            filtered_publishers.append(single_publisher)
    ...
    filtered_authors_quantity = len(filtered_authors)
    filtered_publishers_quantity = len(filtered_publishers)
    ...
    dictionary = {
        ...
        'filtered_authors_quantity': filtered_authors_quantity,
        'filtered_publishers_quantity': filtered_publishers_quantity
        }
...
def reference_detail(request, reference_id):
    ...
    delete_reference = request.GET.get('delete_reference')
    if delete_reference:
        promethee_cursor.execute('''DELETE FROM Author_Reference WHERE reference_id = ?''',(reference_id,))
        promethee_cursor.execute('''DELETE FROM Genre_Reference WHERE reference_id = ?''',(reference_id,))
        promethee_cursor.execute('''DELETE FROM Reference WHERE reference_id = ?''',(reference_id,))
...
def publisher_detail(request, publisher_id):
    ...
    delete_checked = request.GET.get('delete_publisher')
    if delete_checked:
        check_if_publisher_has_references = promethee_cursor.execute('''SELECT reference_id FROM Reference WHERE publisher_reference = ?''', (publisher_id,)).fetchall()
        if check_if_publisher_has_references:
            error = "This publisher is still linked to references and thus cannot be deleted"
            ...
            promethee_cursor.execute('''DELETE FROM Publisher WHERE publisher_id = ?''', (publisher_id,))
            promethee_cursor.close()
            promethee_conn.close()
            return redirect('/search')    
...
def author_detail(request, author_id):
    ...
    delete_author = request.GET.get('delete_author_action')
    if delete_author:
        check_if_author_has_references = promethee_cursor.execute('''SELECT reference_id FROM Author_Reference WHERE author_id = ?''', (author_id,)).fetchall()
        if check_if_author_has_references:
            ...
            promethee_cursor.execute('''DELETE FROM Author WHERE author_id = ?''', (author_id,))
            promethee_cursor.close()
            promethee_conn.close()
            return redirect('/search')
...
def genres(request):
    ...
    searched_keyword_id = request.GET.get('choice')
    if searched_keyword_id:
        ...
        for single_reference_id in references_ids:
            ...
            specific_reference = promethee_cursor.execute('''SELECT * FROM Reference WHERE reference_id = ?''', (reference_id,)).fetchone()
    ...
    if request.method == "POST":
        ...
        check_if_exists_already = promethee_cursor.execute('''SELECT keyword FROM Genre WHERE keyword LIKE ?''', (new_genre,)).fetchall()
        ...
        if check_if_exists_already:
            ...
            promethee_cursor.execute('''INSERT INTO Genre (keyword) VALUES (?)''', (new_genre,))
            promethee_cursor.close()
            promethee_conn.close()
...
def country_detail(request):
    ...
    all_countries = promethee_cursor.execute('''SELECT country_code, population_in_millions, continent, researchers FROM Country ORDER BY country_code ASC''').fetchall()
    countries = []
    ...
    searched_country_code = request.GET.get('choice')
    show_update_form = request.GET.get('update_form')
    if searched_country_code:
        if not show_update_form:
            all_authors_for_country = promethee_cursor.execute('''SELECT author_id, author_firstnames, author_initials, author_lastname, affiliation, author_email FROM Author WHERE country_code = ?''', (searched_country_code,)).fetchall()
            ...
            country_informations = promethee_cursor.execute('''SELECT country_code, population_in_millions, continent, researchers FROM Country WHERE country_code = ?''', (searched_country_code,)).fetchone()
            country_details = {
            ...
            if request.method == "POST":
                ...
                promethee_cursor.execute('''UPDATE Country SET country_code = ?, population_in_millions = ?, continent = ?, researchers = ? WHERE country_code = ?''', (updated_country_code, updated_population_in_millions, updated_continent, updated_researchers, searched_country_code))
    ...
    if request.method == "POST":
        ...
        check_if_exists_already = promethee_cursor.execute('''SELECT country_code FROM Country WHERE country_code = ?''', (new_country_code,)).fetchall()
        ...
        if check_if_exists_already:
            ...
            promethee_cursor.execute('''INSERT INTO Country (country_code, population_in_millions, continent, researchers) VALUES (?, ?, ?, ?)''', (new_country_code, new_population_in_millions, new_continent, new_researchers,))
            promethee_cursor.close()
            promethee_conn.close()
...
def add_reference(request):
    ...
    all_authors = promethee_cursor.execute('''SELECT author_id, author_firstnames, author_initials, author_lastname, country_code, affiliation, author_email FROM Author ORDER BY author_lastname ASC''').fetchall()
    authors = []
    ...
    all_publishers = promethee_cursor.execute('''SELECT publisher_id, publisher_name FROM Publisher ORDER BY publisher_name ASC''').fetchall()
    publishers = []
    ...
    if request.method == 'POST' and authors_count:
        ...
        promethee_cursor.execute('''INSERT INTO Reference (title, publisher_reference, isbn_issn, year, volume, number, pages, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)''', (title, publisher_id, isbn_issn, year, volume, number, pages, type))
        ...
        for position in loop:
            author_id = request.POST.get('selected_author{}'.format(i))
        ...
        for author_id_and_position in authors_ids_and_position:
            ...
            promethee_cursor.execute('''INSERT INTO Author_Reference (author_id, reference_id, position) VALUES (?, ?, ?)''', (single_author_id, reference_id, single_author_position))

        for single_genre_id in selected_genres_ids:
            promethee_cursor.execute('''INSERT INTO Genre_Reference (keyword_id, reference_id) VALUES (?, ?)''', (single_genre_id, reference_id))
            promethee_cursor.close()
            promethee_conn.close()
...
def add_publisher(request):
    ...
    if request.method == 'POST':
        ...
        check_if_already_exists = promethee_cursor.execute('''SELECT publisher_name FROM Publisher WHERE publisher_name = ?''', (new_publisher_name,)).fetchall()
        ...
        if check_if_already_exists:
            ...
            promethee_cursor.execute(''' INSERT INTO Publisher (publisher_name) VALUES (?)''', (new_publisher_name,))
...
def add_author(request):
    ...
    if request.method == 'POST':
        ...
        check_if_author_already_exists = promethee_cursor.execute('''SELECT author_id FROM Author WHERE author_firstnames = ? AND author_initials = ? AND author_lastname = ? AND country_code = ? AND  affiliation = ? AND author_email = ?''', (firstnames, author_initials, lastname, country_code, affiliation, email,)).fetchall()
        ...
        if check_if_author_already_exists:
            ...
            promethee_cursor.execute('''INSERT INTO Author (author_firstnames, author_initials, author_lastname, country_code, affiliation, author_email) VALUES (?, ?, ?, ?, ?, ?)''', (firstnames, author_initials, lastname, country_code, affiliation, email))
            
            author_id = promethee_cursor.execute('''SELECT last_insert_rowid()''').fetchone()[0]

            check_country_code = promethee_cursor.execute('''SELECT country_code FROM Country WHERE country_code = ?''', (country_code,)).fetchall()
            if not check_country_code:
                promethee_cursor.execute('''INSERT INTO Author (author_firstnames, author_intials, author_lastname, country_code, affiliation, author_email) VALUES (?, ?, ?, ?, ?, ?)''', (firstnames, lastname, author_initials, country_code, affiliation, email))
            promethee_cursor.close()
            promethee_conn.close()
...
def update_reference(request, reference_id):
    ...
    genres = promethee_cursor.execute('''SELECT keyword_id, keyword FROM Genre ORDER BY keyword ASC''').fetchall()
    all_genres = []
    for single_genre in genres:
        genre = {
            'keyword_id': single_genre[0],
    ...
    specific_genres = []
    for single_genre_id in genre_ids:
        genre_name = promethee_cursor.execute('''SELECT keyword FROM Genre WHERE keyword_id = ?''', (single_genre_id[0],)).fetchall()[0][0]
        single_genre = {
            'genre_id': single_genre_id[0],
    ...
    for single_author_id_position in authors_id_position:
        single_author_id = single_author_id_position[0]
        single_author_position = single_author_id_position[1]
        author_identity = promethee_cursor.execute('''SELECT author_firstnames, author_initials, author_lastname, country_code, affiliation, author_email FROM Author WHERE author_id = ?''', (single_author_id,)).fetchall()[0]
        single_author_informations = {
            'author_id': single_author_id,
            'author_position': single_author_position,
    ...
    current_reference = promethee_cursor.execute('''SELECT reference_id, title, publisher_reference, isbn_issn, year, volume, number, pages, type FROM Reference WHERE reference_id = ?''', (reference_id,)).fetchall()[0]

    publisher_name = promethee_cursor.execute('''SELECT publisher_name FROM Publisher WHERE publisher_id = ?''', (current_reference[2],)).fetchall()[0][0]
    ...
    if request.method == "POST":
        ...
        promethee_cursor.execute('''UPDATE Reference SET title = ?, publisher_reference = ?, isbn_issn = ?, year = ?, volume = ?, number = ?, pages = ?, type = ? WHERE reference_id = ?''', (new_title, new_publisher_selected, new_isbn_issn, new_year, new_volume, new_number, new_pages, new_type, reference_id))
        ...
        authors_ids_and_position = []
        for position in loop:
            new_author_id = request.POST.get('selected_author{}'.format(i))
            single_author_id_and_position = {
            ...
            authors_ids_and_position.append(single_author_id_and_position)
            i += 1
            promethee_cursor.execute('''UPDATE Author_Reference SET author_id = ?, reference_id = ?, position = ? WHERE reference_id = ? AND position = ?''',
...
def update_publisher(request, publisher_id):
    ...
    if request.method == "POST":
        ...
        promethee_cursor.execute('''UPDATE Publisher SET publisher_name = ? WHERE publisher_id = ?''', (new_publisher_name, publisher_id))
        promethee_cursor.close()
        promethee_conn.close()
        return redirect('/publisher/{}'.format(publisher_id))
...
def update_author(request, author_id):
    ...
    if request.method == "POST":
        ...
        promethee_cursor.execute('''UPDATE Author SET author_firstnames = ?, author_initials = ?, author_lastname = ?, country_code = ?, affiliation = ?, author_email = ? WHERE author_id = ?''', (new_firstnames, author_initials, new_lastname, new_country_code, new_affiliation, new_email, author_id))
        promethee_cursor.close()
        promethee_conn.close()
        return redirect('/author/{}'.format(author_id))
